...
with ctrl_atk:
    ...
    # ── Autoencoder Boundary Explorer ─────────────────────────────────────────────
    st.markdown("---")
    st.markdown(f"<h4 style='color:{THEME['cyan']}'>🎛️ Autoencoder Boundary Explorer</h4>", unsafe_allow_html=True)
    ...
    if ae_model is None:
        st.warning("Model not trained yet — run python run.py train first")
    else:
        ...
        try:
            ...
            if top_5:
                if mse > cfg.MSE_THRESHOLD_MEDIUM:
                    ...
                    explanations = {
                        "flow_bytes_per_sec": "High data transfer rate detected — consistent with data exfiltration or DDoS.",
                        "Flow Bytes/s": "High data transfer rate detected — consistent with data exfiltration or DDoS.",
                        "fwd_packet_len_mean": "Unusual forward packet sizes observed — potential payload injection.",
                        "Fwd Pkt Len Mean": "Unusual forward packet sizes observed — potential payload injection.",
                        "bwd_packet_len_mean": "Abnormal backward packet sizes — possible command-and-control (C2) instruction delivery.",
                        "Bwd Pkt Len Mean": "Abnormal backward packet sizes — possible command-and-control (C2) instruction delivery.",
                        "flow_iat_mean": "Irregular inter-arrival times — indicative of automated scanning or beaconing.",
                        "Flow IAT Mean": "Irregular inter-arrival times — indicative of automated scanning or beaconing.",
                        "syn_flag_count": "Excessive SYN flags detected — likely a SYN flood or aggressive port scan.",
                        "SYN Flag Count": "Excessive SYN flags detected — likely a SYN flood or aggressive port scan.",
                        "dst_port": "Traffic aimed at unusual ports — strong indicator of lateral movement.",
                        "Destination Port": "Traffic aimed at unusual ports — strong indicator of lateral movement."
                    }
